...
tableElement.addEventListener("click", (e)=>{
    ...
    if (prevSortBy === thElement) {
        prevSortBy.dataset.sorted = prevSortBy.dataset.sorted === "ask" ? "desk" : "ask";
        sortTableBy(users, thElement.textContent, prevSortBy.dataset.sorted);
    } else {
        if (prevSortBy) prevSortBy.removeAttribute("data-sorted");
        thElement.dataset.sorted = "ask";
        sortTableBy(users, thElement.textContent);
    }
    const sortedTableRows = users.map(({ Name, Position, Office, Age, Salary })=>{
        return `<tr>
        <td>${Name}</td>
        <td>${Position}</td>
        <td>${Office}</td>
...
function sortTableBy(arr, property, order = "ask") {
    if (property === "Name" || property === "Position" || property === "Office") arr.sort((u1, u2)=>{
        if (order === "ask") return u1[property].localeCompare(u2[property]);
        return u2[property].localeCompare(u1[property]);
    });
    else if (property === "Age") arr.sort((u1, u2)=>{
        if (order === "ask") return u1[property] - u2[property];
        ...
        if (order === "ask") return salary1 - salary2;
...
tableElement.addEventListener("click", (e)=>{
    ...
    if (!rowElement.classList.contains("active")) rowElement.classList.add("active");
});
// 3-4 tasks
const newEmployeeForm = document.createElement("form");
newEmployeeForm.classList.add("new-employee-form");
newEmployeeForm.insertAdjacentHTML("beforeend", `<label>
    Name:
...
newEmployeeForm.insertAdjacentHTML("beforeend", '<button type="submit">Save to table</button>');
...
newEmployeeForm.addEventListener("submit", (e)=>{
    ...
    const notifications = [];
    if (!userName || !position || !office || !age || !salary) notifications.push({
        title: "Missing data",
        description: "All fields are required."
    });
    if (!!userName && userName.length < 4) notifications.push({
        title: "Invalid name",
        description: "Name should be have at least 4 letters."
    });
    if (!!age && (age < 18 || age > 90)) notifications.push({
        title: "Invalid age",
...
tableElement.addEventListener("dblclick", (e)=>{
    ...
    if (!td || currentlyEditingCell) return;
    const originalText = td.textContent;
    const columnIndex = td.cellIndex;
    const headerText = td.closest("table").querySelectorAll("th")[columnIndex].textContent.trim();
    td.textContent = "";
    ...
    const saveAndCleanup = ()=>{
        ...
        const notifications = [];
        if (headerText === "Name" && newValue.length < 4) notifications.push({
            title: "Invalid name",
            description: "Name should have at least 4 letters."
        });
        if (headerText === "Position" && newValue === "") notifications.push({
        ...
        if (headerText === "Office" && !cities.includes(newValue)) notifications.push({
            title: "Invalid office",
            description: "Office should be one of the predefined cities."
        });
        if (headerText === "Age") {
            const num = Number(newValue);
            if (num < 18 || num > 90) notifications.push({
                title: "Invalid age",
                description: "Age should be between 18 and 90."
            });
        }
        if (headerText === "Salary") {
            const num = Number(newValue);
            if (isNaN(num) || num <= 0) notifications.push({
        ...
        if (notifications.length > 0) {
            ...
            pushNotification("error", notifications);
        } else td.textContent = headerText === "Salary" ? `$${formatBigNumber(newValue)}` : newValue;
